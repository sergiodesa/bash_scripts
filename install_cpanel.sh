...
echo "  __      __         __________                    .__    .___      "
echo " /  \    /  \ ____   \______   \_______  _______  _|__| __| _/____  "
echo " \   \/\/   // __ \   |     ___/\_  __ \/  _ \  \/ /  |/ __ |/ __ \ "
echo " 	\        /\  ___/   |    |     |  | \(  <_> )   /|  / /_/ \  ___/ "
echo "   \__/\  /  \___  >  |____|     |__|   \____/ \_/ |__\____ |\___  >"
echo "        \/       \/                                        \/    \/ "
...
if [ ! -f /etc/redhat-release ]; then
	echo "CentOS nao detectado, PAREI!!!"
	exit 0
fi

echo "Este script instala e pre-configura cPanel (CTRL + C para cancelar)"
sleep 10

echo "####### CONFIGURANDO CENTOS #######"
wget http://dl.wp-ns.com/scripts/configure_centos -O "$CWD/configure_centos.sh" && bash "$CWD/configure_centos.sh"

echo "####### PRE-CONFIG CPANEL ##########"
...
echo "Alterar o runlevel a 3..."
systemctl isolate runlevel3.target
systemctl set-default runlevel3.target

echo "####### INSTALAR CPANEL #######"
if [ -f /usr/local/cpanel/cpanel ]; then
        echo "cPanel detectado, não vai ser instalado, só configura (CTRL + C para cancelar)"
        sleep 10
else
...
if [ ! -d /etc/csf ]; then
    echo "csf no instalado, a fazer o download!"
...
echo "Activando range passiva FTP..."
# IPv4
...
echo "Abrir portas para migrações"
...
echo "####### FIM CSF #######"
echo "####### CONFIGURAR CPANEL #######"

if [ ! -d /usr/local/cpanel ]; then
	echo "cPanel não encontrado. I'm OUT!!!"
	exit 0
fi

HOSTNAME_LONG=$(hostname -d)

echo "Reduzir TTL de DNS a 1 Hora..."
sed -i 's/TTL 14400/TTL 3600/' /etc/wwwacct.conf

#echo "Alterar mail de contacto..."
...
# CONFIGURAÇÕES QUE N DÃO PELA CONSOLA
echo "Configs que não dão pela consola, mas sim pela API"
yum install -y curl
...
echo "Config valores default PHP-FPM..." # https://documentation.cpanel.net/display/74Docs/Configuration+Values+of+PHP-FPM
...
if [ $ISVPS = "NO" ]; then
	...
	echo "Disable regras que causam conflito..."
...
echo "Update para MariaDB 10.2..."
whmapi1 start_background_mysql_upgrade version=10.2

echo "Config feature disabled..."
...
echo "Activar Header Authorization em CGI..."
sed -i '/# INICIO ACTIVAR HEADER AUTHORIZATION CGI/,/# FIM ACTIVAR HEADER AUTHORIZATION CGI/d' /etc/apache2/conf.d/includes/pre_main_global.conf

cat >> /etc/apache2/conf.d/includes/pre_main_global.conf << 'EOF'
# INICIO ACTIVAR HEADER AUTHORIZATION CGI
SetEnvIf Authorization "(.*)" HTTP_AUTHORIZATION=$1
# FIM ACTIVAR HEADER AUTHORIZATION CGI
...
echo "Config JailShell..."
echo "/etc/pki/java" >> /var/cpanel/jailshell-additional-mounts

echo "Misc..."
chmod 755 /usr/bin/wget # NO WGET FOR EVERYONE

history -c
echo "" > /root/.bash_history

echo "#### Done, o MYSQL ainda está a actualizar!!!!###"
